...
document.getElementById('pauseSongBtn').addEventListener('click', function() {
    if (currentAudio) {
        currentAudio.pause();
    }
});
...
document.getElementById('skipSongBtn').addEventListener('click', function() {
    ...
    const nextIndex = (selectedIndex + 1) % songSelect.options.length; // Loop back to the first song
    songSelect.selectedIndex = nextIndex; // Update the selected index
    const nextSong = songSelect.value; // Get the next song
    currentAudio = new Audio(nextSong);
    currentAudio.play(); // Play the next song
});
